...
var c = document.createElement("canvas"); // for gridlines
...
mapRange.ToWebGL = {
    x: function(s) {return mapRange([0, c.width], [-1, 1], s);},
    y: function(s) {return mapRange([0, c.height], [-1, 1], s);}
};
...
var colors = {
    ...
    random: new WebglPlotBundle.ColorRGBA(Math.random(), Math.random(), Math.random(), 1),
    fromHex: function(h) {
        var r = 0, g = 0, b = 0;
        // 3 digits
        if (h.length == 4) {
...
// 4. Global variables
const wglp = new WebglPlotBundle.WebglPlot(c);
const midPoint = {x: c.width/2, y: c.height/2};
...
function createGrid(){
    ...
    while (gridLineX >= 0){
      addLine.fromCanvas([gridLineX, 0], [gridLineX, c.height]);
      gridLineX -= 100;
    }
    gridLineX = midPoint.x + 100;
    ...
    while (gridLineY >= 0) {
        addLine.fromCanvas([0, gridLineY], [c.width, gridLineY]);
        gridLineY -= 100;
    }
    gridLineY = midPoint.y + 100;
...
var fullSampledTime = function() {
    return Math.ceil(c.width/100)*parseFloat(ui.timeScale.value)/1000;
}
...
function drawData(){
    ...
    if (generator_refresh) {
        for (var i = 0; i < line.numPoints; i++) {
            var xc = i * (c.width/line.numPoints);
...
wglp.addLine(line2);
line2.visible = false;
const square = new WebglPlotBundle.WebglSquare(colors.bg);
square.setSquare(1,-1,1,1);
wglp.addSurface(square);
wglp.update();
